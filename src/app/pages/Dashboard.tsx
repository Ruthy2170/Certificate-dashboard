import { Trophy, Star, CheckCircle2, Calendar, Download, Award, Shield, TrendingUp } from 'lucide-react';
...
export default function Dashboard() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      {/* Welcome Header */}
      <div className="mb-8">
        <h1 className="mb-2 text-3xl font-bold text-gray-900">Welcome back, {userData.name}! 👋</h1>
      ...
      <div className="mb-8 grid gap-6 sm:grid-cols-1">
        ...
        <div className="group rounded-2xl bg-gradient-to-br from-purple-500 to-purple-600 p-6 shadow-lg transition-all hover:scale-105 hover:shadow-xl">
          ...
          <p className="mb-1 text-sm font-medium text-purple-100">Certificates Earned</p>
      ...
      <div className="mb-8 rounded-2xl bg-white p-6 shadow-lg">
        <div className="mb-6 flex items-center justify-between">
          ...
          <button className="text-sm font-medium text-blue-600 hover:text-blue-700">See Timeline</button>
        </div>
        <div className="space-y-4">
          {achievements.map((achievement) => {
            const IconComponent = achievement.icon;
            return (
              <div key={achievement.id} className="group flex items-center gap-4 rounded-xl border border-gray-100 p-4 transition-all hover:border-blue-200 hover:bg-blue-50/50">
                <div className={`flex size-14 shrink-0 items-center justify-center rounded-xl ${achievement.iconBgColor} shadow-lg`}>
                  <IconComponent className="size-7 text-white" />
                </div>
                <div className="flex-1">
                  <div className="mb-1 flex items-center gap-2">
                    <h3 className="font-semibold text-gray-900">{achievement.title}</h3>
                    <span className="rounded-full bg-green-100 px-2 py-0.5 text-xs font-semibold text-green-700">+{achievement.points} pts</span>
                  </div>
                  <p className="mb-1 text-sm text-gray-600">{achievement.description}</p>
        ...
        <div className="grid gap-6 sm:grid-cols-2">
          {certifications.map((cert) => (
            ...
            >
              ...
              <p className="mb-4 text-sm font-medium text-gray-600">{cert.organization}</p>
